...
def get_pdfkit_config():
    ...
    raise FileNotFoundError(
        "wkhtmltopdf not found! Install from: https://wkhtmltopdf.org/downloads/"
    )
...
def inject_user():
    if current_user.is_authenticated:
        ...
        try:
            u = conn.execute(
                "SELECT coin_balance FROM users WHERE id = ?", (current_user.id,)
            ).fetchone()
        finally:
            conn.close()
...
def register():
    if request.method == "POST":
        ...
        try:
            ...
            cur = conn.execute(
                "INSERT INTO users (username, email, password, coin_balance) VALUES (?, ?, ?, ?)",
                (username, email, hashed, 100),
            )
            conn.commit()
            uid = cur.lastrowid
            conn.execute(
                "INSERT INTO coin_transactions (user_id, amount, transaction_type, description) VALUES (?, ?, ?, ?)",
...
def restaurant(restaurant_id):
    ...
    # Build menu categories safely
    menu_by_category = {}
    for item in menu_items:
        menu_by_category.setdefault(item["category"], []).append(item)

    return render_template(
        "restaurant.html", restaurant=r, menu_by_category=menu_by_category
    )
            ...
            if coins_to_use > 0:
                conn.execute(
                    ...
                    (
                        ...
                        -coins_to_use,
                        "spent",
                        f"Used on order #{order_id}",
                    ),
                )
            cashback_coins = int(total * 0.05)
            conn.execute(
                "UPDATE users SET coin_balance = coin_balance + ? WHERE id = ?",
                (cashback_coins, current_user.id),
            )
            conn.execute(
                "INSERT INTO coin_transactions (user_id, amount, transaction_type, description) VALUES (?, ?, ?, ?)",
                (
                    ...
                    cashback_coins,
                    "earned",
                    f"Cashback from order #{order_id}",
                ),
            )
            conn.commit()
            session.pop("cart", None)
            flash(
                f"Order placed successfully! You earned {cashback_coins} cashback coins!",
                "success",
            )
...
def wallet():
    ...
    try:
        ...
        conn.close()
    return render_template(
        "wallet.html", transactions=transactions, coin_balance=user["coin_balance"]
    )
...
def add_vendor():
    if request.method == "POST":
        ...
        image_url = ""
        # store a web-friendly path or empty string
